...
def uploaded_file(event_id, filename):
	...
	if not os.path.exists(event_folder):
		return abort(404, description="Event folder not found") # type: ignore
...
def run_yolo_face_tracking_webcam(event_id, mysql, threshold_dist=0.7):
	...
	while True:
		...
		results = model.track(frame, persist=True, tracker="custom_botsort.yaml", conf=0.5 )
			...
			with app.app_context():
				...
				for box, id in zip(boxes, ids):
					...
					for invitee in invitees:
						...
						result = DeepFace.verify(cropped_image, os.path.join(event_folder, reference_image), model_name='Facenet512', detector_backend = 'skip')
					...
					if not match_found:
						...
						cropped_face_path = os.path.join(event_folder, unknown_filename)
...
def check_event_timing_and_track(event_id, mysql):
	with app.app_context():
		...
		end_time_total_seconds = event_end_time.total_seconds()
		end_hours = int(end_time_total_seconds // 3600) % 24
		end_minutes = int((end_time_total_seconds % 3600) // 60)
		end_seconds = int(end_time_total_seconds % 60)
		...
		start_time_total_seconds = event_start_time.total_seconds()
		start_hours = int(start_time_total_seconds // 3600) % 24
		start_minutes = int((start_time_total_seconds % 3600) // 60)
		start_seconds = int(start_time_total_seconds % 60)
...
#run_yolo_face_tracking_webcam('1xbBtagrVVvJ', mysql)
...
def register_event():
	try:
		...
		cursor.execute('''
			INSERT INTO events (event_id, event_name, event_date, start_time, end_time, location, organisation_id, threshold)
			VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
		''', (event_id, event_name, event_date, start_time, end_time, location, organisation_id, threshold))
		mysql.connection.commit()
		cursor.close()

		# Trigger tracking thread right after the event is created
		start_tracking_thread(event_id, mysql)
	...
	try:
		...
		if event:
			...
			for (x, y, w, h) in faces:
				...
				cropped_face_path = os.path.join(event_folder, cropped_filename)
...
def get_attendees(event_id):
	try:
		...
		event_start_time = results[0][2] 
		event_end_time = results[0][3]   
		event_threshold = results[0][4]  
			...
			if current_time > event_end_datetime:  # Compare time objects directly
				...
				if timestamps:
					...
					for arrival_str, departure_str in zip(timestamps['arrivals'], timestamps['departures']):
						...
						# Combine time with date to calculate the duration correctly
				...
				attended = total_duration >= timedelta(seconds=event_threshold)

				if attended and not isAttended:
					cursor = mysql.connection.cursor()
					cursor.execute('UPDATE invitees SET isAttended = TRUE WHERE invitee_id = %s', (row[5],))
					mysql.connection.commit()
					cursor.close()
				...
				attendee_dict = {
					...
					'photo': row[8],
					'timestamps': row[9],
					'isAttended': False,  # Attendance not computed yet
